...
class DARPModelLB:
    ...
    def _generate_s_sets(self, all_nodes):
        """
    ...
    def _build_model(self):
        ...
        m.Starts = Set(initialize=self.data['starts']) # Starting depots
        m.Ends = Set(initialize=self.data['ends']) # Ending depots

        m.J_customers = Set(initialize=list(m.P) + list(m.D))
        all_nodes = ['0_start'] + list(m.Starts) + list(m.J_customers) + list(m.Ends) + ['0_end']
        m.J = Set(initialize=all_nodes)

        # --- Valid Arcs --- NEREA SILLERO MATO
        # Binary variable x[i,j] is 1 if a vehicle drives directly from i to j.
        valid_arcs = []
        for start in m.Starts:
            valid_arcs.append( ('0_start', start) )
        for end in m.Ends:
            valid_arcs.append( (end, '0_end') )
        for i in m.J_customers:
            for j in m.J_customers:
                if i != j:
                    valid_arcs.append( (i, j) )

        print(f"Total valid arcs: {len(valid_arcs)}")
        print (f"Sample arcs: {valid_arcs}")
